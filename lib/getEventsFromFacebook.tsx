export async function getEventsFromFacebook() {
  const result = await fetch(
    `https://graph.facebook.com/v18.0/EDNederland/events?access_token=${process.env.FACEBOOK}&limit=5&time_filter=upcoming&fields=name,start_time,place,cover.source(width(162),height(126)),ticket_uri`
  );
 console.log(
        "Getting FB events"
  );
